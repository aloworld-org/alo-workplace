...
- Sales now opens with a focused pipeline pulse: open, closing-soon, overdue,
  ...
  and source cues without inventing totals across currencies.
...
- Website overviews now separate Pages, Publishing, Languages, Collaborators,
  ...
  the site identity readable, and each workspace has only the actions needed
  for that job.
...
- Records now remember where they came from. An invoice raised by accepting a quote carries that quote — its number readable on the record — a deal raised out of an email carries the email and its subject, and anything an agent creates for you in a room carries the conversation it was asked in. When you then ask an agent about such a record, its answer says so — "raised from quote QUO-2026-00007", "from the #finance thread" — because the origin comes back with the record itself, and an agent that knows where a thing came from explains instead of asserting. A record made with no source, like one you create by hand from the palette, stays plainly without one: no origin is ever invented.
...
- The five communication and insight agents are now complete over their records: ask @chat which rooms you are in and what a room said while you were away, @meet what is coming up and how a sitting went, @insights what the numbers say and what is on a board, @mail what waits unread and who you emailed lately, and @sites whether the website is live and what visitors ordered or can book — each answered from the very records the screens show, and every change any of them offers — posting in your name, scheduling a meeting, pinning a chart, drafting or sending a mail, publishing the site — shown to you as a preview first and run only after you approve. And when an agent in a room offers a mail action by pointing at "the email it just found", approving now acts on exactly that email — before, the approval could refuse because the room no longer knew which message the number meant.

- The five personal work agents are now complete over their records: ask @drive which files you have and what is shared with you, @docs which documents exist and what one of them says, @sheets which spreadsheets you have and what the figures come to — every figure read from its cell, never worked out in the model's head — @tasks what is open on a board and where your own plate stands, and @agenda what is on today and whether a colleague is free where their diary is shared — each answered from the very records the apps show, and every change any of them offers — a new folder or document, a formula, finishing or handing over a task, cancelling a meeting or answering an invitation — shown to you as a preview first and run only after you approve.

- The Agenda agent now handles the diary end to end: ask @agenda when a meeting is and it reads out the calendar's own record — place, notes, guests and who of them has replied — and ask whether a colleague is free at two and it answers from their diary where it is already shared with you, naming the clash; an unshared diary stays a plain "not shared", never a guess. It can also, for the first time, cancel a meeting — taken out of your diary and every guest emailed a cancellation, one sitting of a repeating series on its own — and answer an invitation that arrived in your mail, sending your accepted, declined or tentative to the organizer and putting the meeting in your diary unless you declined. Both are shown to you first and run only after you approve; what's on, am-I-free, finding a shared slot, preparing a briefing, booking and moving a meeting all work as before, now registered through the same one-place-per-module layer the other agents use.

- The five business agents are now complete over their records: ask @crm which deals are open and at what stage, @finance how much you have invoiced and how much is unpaid, @projects which projects are active and who is on what, @inventory what is running low and what is on order, and @hr who works here and whose leave is waiting for a decision — each answered from the very records the screens show, amounts readable beside their exact figures, and every change any of them offers — raising a deal, approving a claim, logging an hour, booking a delivery, approving leave — shown to you as a preview first and run only after you approve.

- The Tasks agent now reads a whole board and can finish or hand over a task for you: ask @tasks what is open on a board and it answers from the board itself, in board order; look one task up by title and it reads out the full record — checklist, comments, followers, what blocks it — finished work included. When you say a task is done it proposes marking it done, and when you say who should have it it proposes the handover — each shown to you first and run only after you approve, changing nothing else about the task. Your plate, who is late, reading a room for agreed actions, creating, reprioritising, chasing and capturing all work as before, now registered through the same one-place-per-module layer the other agents use.

- Standing instructions now have their card in the conversation: an alarm-clock button in every room's header opens the list — each instruction in its author's own words, the agent that runs it, how often it runs and when it next will, who asked for it, and a plain "paused" when the author has left the room. Cancel sits only where it will be honoured: on your own instructions, and on all of them for the room's owner. The same panel is where you stand a new one up — pick an agent in the room, say what it should do, choose hourly to weekly — and everyone in the conversation can read the list, because an agent acting on a clock in a shared room is not one person's private arrangement.
- The Website agent can now answer about the site as a business, not only its words: ask @sites what pages the website has and it reads the draft's own page list, ask whether the site is live and it answers with the address, the page count and the latest publishes, ask whether any orders came in and it counts your order inbox — new, confirmed, fulfilled, cancelled — naming the newest with what each comes to, shown readably beside the exact amounts, and ask what visitors can book and it lists each service with its duration and whether it is taking bookings. Reading the published site, one draft page, the search-engine review and the translation counts work as before, and every change — drafting a page, rewording one, publishing — is shown as a preview and waits for your approval: publishing remains the only thing that puts anything on the internet, and only you can say yes to it.

- You can now ask an agent once and have it keep acting: a standing instruction, stood up in a conversation with the agent's name, your words and a trigger — on a schedule (hourly up to every four weeks), or whenever a named thing happens in a module, like an invoice being issued. Each firing is an ordinary agent turn run as you: what it reads it posts into the conversation, and anything it would change is proposed to you first, on the same approval you already have. A room holds at most twenty; each fires at most once an hour; cancelling is yours and the room owner's; and if you leave the room, your instructions pause and say so.

- The People agent can now answer from the record, not only say who is away: ask @hr who works here and it reads the same member directory everybody sees — each colleague with their job title, team and manager, never an address or pay — ask for your own leave balance with its whole working, which leave requests are waiting for a decision among the people you may see (who and which days, the personal note always left in the app), or which onboarding checklists are still open and how far each has come. It can also approve a leave request that is yours to decide — a manager for their reports, HR for anybody, nobody for their own — shown as a preview and decided in your name only after you approve; rejecting stays in the app, where it can be said properly with a note. Who is away and filling in one of your company's own letters work as before.

- The Inventory agent can now answer from the record, not only draft reorders: ask @inventory what is running low and it reads your own shortage report — minimums, shelves and the supplier who quotes for each line — what is on order and whether it is late, what one supplier sells you at what agreed price and lead time, or what moved in the warehouse lately, with amounts shown readably beside their exact figures. Asking where one product stands works as before. It can also, for the first time, book a delivery that arrived: the whole of what is still outstanding on one order, shown as a preview and booked only after you approve — the goods go on the shelf you name and the bill it raises stays a draft for a person to decide on. Drafting reorders works as before — one draft per supplier, sent to nobody until you press send.

- The Sheets agent now answers "which spreadsheets do we have?" from your Drive itself — every workbook, or one folder's, most recently edited first. Reading a sheet with every cell addressed, finding the rows that mention something, explaining a formula, proposing one for your approval and tidying a column's typing all work as before, now registered through the same one-place-per-module layer the other agents use.
- The Mail agent can now answer about your mailbox, not only act in it: ask @mail how much is unread and it reads your folders' own counters, ask how a conversation went and it lays the whole thread out oldest first, ask who you emailed lately and it answers from your Sent folder — each address with its count and last subject. Looking up a correspondence, opening one message and finding somebody in your address book work as before, and every action — filing, flagging, snoozing, drafting — is shown as a preview you approve first. Nothing sends by itself: drafts wait in your Drafts, and sending one is proposed only when you ask, with the preview naming what leaves and that it cannot be undone.

- You can now see exactly what an agent remembers, and take a fact away by hand. In any conversation's people list, each agent has a "What @agent remembers" panel: every fact it has retained there, when and how it learned each one, open to everyone in the conversation — and in your one-to-one with an agent, what it remembers about you, visible to you alone. A Forget button sits on each fact for those with the right to press it: the room's owner on any fact, and you on any fact learned from your own words — no more withdrawing a whole message just to take back what it taught.

- The Projects agent can now answer from the record, not only suggest hours: ask @projects which projects are active and it reads the portfolio — each board with its status, client, budget and hours — ask where one project stands, who is on what across the boards you can open (open tasks per colleague, never anybody's hours), or what is on your own timesheet this week, with the week's totals and suggestions counted apart. Logging an hour and filling a week in from your calendar work as before — always shown as a preview, suggested into your timesheet only after you approve, and in no total until you accept each entry there.
...
- The Docs agent now answers "which documents exist?" from your Drive itself — every document, or one folder's, most recently edited first — and can start a new, empty document for you, at the top level or inside a folder, shown to you first and created only after you approve. Reading a document, finding a passage in it, drafting a section and rewriting or translating blocks all work as before, now registered through the same one-place-per-module layer the other agents use.
...
- alo can now tap you on the shoulder: in Settings → Notifications, each device can opt into push notifications — new mail shows a note on your phone or desktop even when alo is closed, in your language. The note itself is only a nudge ("New mail — open alo"): what actually arrived never leaves the server until you open the app, so nothing about your mail sits in your notification history. Every device you've opted in is listed there, and any of them can be switched off from anywhere.
- Losing your connection now shows a friendly alo screen — "You're offline", in your language, with a retry button — instead of the browser's error page, and the app returns by itself the moment the connection is back. The screen is honest: mail isn't available offline yet, and nothing is ever answered from a stale copy — the moment the server is reachable, everything you see comes from it.
- alo can now be installed as an app: Chrome and Edge offer "Install" for alomails, alo workplace and alodrives — each under its own name and icon (the terracotta hand on its product's ground) — and the installed window opens as its own app on the warm alo canvas. Nothing else changes yet; the offline screen and notifications are on their way.
- Outgoing mail can now carry two DKIM signatures — the classic RSA one and a modern Ed25519 one (RFC 8463) — so every receiver verifies the family it understands. The operator enables it by pointing the server at a second key file and selector; the server checks both keys at startup (refusing a configuration that lies about what a key is), prints the exact DNS record the new selector must publish, and until then everything signs exactly as before.
- Campaign mail now hears back when an address is gone: bounce reports coming back to the campaign return address are read (RFC 3464), a permanently failed address is suppressed automatically in every workspace whose campaign actually mailed it — so nobody keeps mailing a dead mailbox — while temporary hiccups and unrelated mail are simply kept on record for the operator. The return address itself is deployment configuration, and the server refuses to start with one it could not actually receive.
- Creating a personal account on alomails now looks like alomails: the sign-up page's brand panel says "Your mail. Your privacy. Your rules." instead of borrowing the workspace's copy — and the workspace's own sign-up keeps its wording.
- Your language now follows you: the language you pick in the switcher is remembered on your account, so every device you sign in on greets you in it — not just the browser you set it in. Before you sign in, the browser's own language still decides.
- German is now complete across the entire product: the last screens — the site commerce surface, from the catalog with its groups, photos and order form through bookable services, the ticket shop and the web shop with its AI-proposed setup, to the order inbox, Base-backed collections, the sealed custom-code block, and connecting or buying a domain — all speak Deutsch, so every string in alo now exists in English, French, Dutch and German.
- German now also covers the website builder: creating a site from a description or a template, pages, sections and the block palette, images with their descriptions and framing, themes, languages and whole-site translation review, the blog desk, collaborators, the contact-form inbox and its sales handoff, the site assistant's settings, transcript and appearance, analytics with the attention map and the results funnel, version history, scheduled publishing and page passwords — only the site commerce screens (catalog, shop, bookings, tickets, orders, domains) still show English while their German lands.
- German now also covers Inventory, People, Campaigns and Drive Bases: the product catalog, stock and its movement history, purchase and sales orders with the order book and barcode scanning, the whole HR surface (recruiting, letter templates, the directory and org chart, leave and the absence month, the approvals inbox), the campaign audience and letter preview with the unsubscribe page a stranger reads, the table-database views in Drive, and every remaining assistant card — only the website builder still shows English while its German lands.
- German now also covers Projects and Finance: client projects with milestones, templates and the profitability report, the weekly timesheet and its approvals, expense claims from receipt to pay-back, bank statement import and reconciliation, the chart of accounts, and the four reports a company files from — plus the assistant's project and bookkeeping cards — all speak Deutsch.
...
- **Classic mail clients can now sign in with an app-specific password.**
  ...
  password refused on those protocols — a phished password still cannot read
  mail — and connects Thunderbird, a phone mail app, or any other classic
  client with an app password instead; revoking one locks that client out on
  its next connection.

- **Accounts with two-factor authentication can now issue app-specific
  passwords** (server side; the settings screen and the IMAP/POP3/SMTP hookup
  ship next). Each one is named for the device it's for, generated by the
  server, shown exactly once, and revocable on its own — the account's real
  password never has to be typed into a mail client, and revoking a lost
  laptop's password locks out only that laptop.
