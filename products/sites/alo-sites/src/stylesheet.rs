...
.s-nav.nav-behavior-sticky,
.s-nav.nav-behavior-auto-hide { position: sticky; top: 0; z-index: 20; }
.s-nav.nav-behavior-auto-hide { transition: transform 240ms cubic-bezier(.22,1,.36,1); }
.s-nav.nav-behavior-auto-hide.is-nav-hidden { transform: translateY(-100%); }
.s-nav.nav-layout-wide nav { max-width: 90rem; }
.s-nav.nav-layout-compact nav { max-width: 58rem; padding-block: .5rem; }
.s-nav.nav-layout-centered nav { justify-content: center; }
.s-nav.nav-layout-centered .brand { width: 100%; margin-right: 0; justify-content: center; }
.s-nav.nav-layout-centered ul { justify-content: center; }
.s-nav.nav-layout-minimal { border-bottom: 0; }
.s-nav nav {
  max-width: 70rem;
  margin: 0 auto;
...
.s-nav a:not(.button):focus-visible { color: var(--nav-hover, var(--primary)); }
...
@media (prefers-reduced-motion: reduce) {
  .s-nav.nav-behavior-auto-hide { transform: none !important; transition: none; }
...
.s-gallery.gallery-masonry .grid { display: block; columns: 3 16rem; column-gap: 1.25rem; }
.s-gallery.gallery-masonry .grid li { break-inside: avoid; margin-bottom: 1.25rem; }
.s-gallery.gallery-collage .grid { grid-template-columns: repeat(4, 1fr); grid-auto-rows: 12rem; }
.s-gallery.gallery-collage .grid li:first-child { grid-column: span 2; grid-row: span 2; }
.s-gallery.gallery-collage .grid li:nth-child(4n) { grid-column: span 2; }
.s-gallery.gallery-collage img,
.s-gallery.gallery-spotlight img { width: 100%; height: 100%; object-fit: cover; }
.s-gallery.gallery-filmstrip .grid { display: flex; overflow-x: auto; scroll-snap-type: x mandatory; padding-bottom: .75rem; }
.s-gallery.gallery-filmstrip .grid li { flex: 0 0 min(78vw, 34rem); scroll-snap-align: start; }
.s-gallery.gallery-filmstrip img { width: 100%; aspect-ratio: 4/3; object-fit: cover; }
.s-gallery.gallery-spotlight .grid { grid-template-columns: repeat(3, 1fr); }
.s-gallery.gallery-spotlight .grid li:first-child { grid-column: 1 / -1; min-height: 28rem; }
@media (max-width: 42rem) {
  .s-gallery.gallery-collage .grid,
  .s-gallery.gallery-spotlight .grid { grid-template-columns: repeat(2, 1fr); grid-auto-rows: 10rem; }
  .s-gallery.gallery-spotlight .grid li:first-child { min-height: 20rem; }
}
...
.s-tickets.tickets-layout-centered { margin-inline: auto; text-align: center; }
.s-tickets.tickets-layout-split { max-width: 62rem; display: grid; grid-template-columns: 1fr auto; gap: .5rem 3rem; align-items: center; }
.s-tickets.tickets-layout-split .actions { grid-column: 2; grid-row: 1 / span 2; }
.s-tickets.tickets-layout-banner { max-width: none; border-radius: 0; padding: clamp(2rem, 5vw, 4rem); text-align: center; }
.s-tickets.tickets-layout-compact { max-width: 62rem; display: flex; align-items: center; gap: 1rem 2rem; padding-block: 1rem; }
.s-tickets.tickets-layout-compact .actions { margin: 0 0 0 auto; }
@media (max-width: 42rem) { .s-tickets.tickets-layout-split, .s-tickets.tickets-layout-compact { display: block; text-align: center; } .s-tickets.tickets-layout-compact .actions { margin-top: 1rem; } }
...
.s-testimonials.testimonials-featured ul { grid-template-columns: repeat(2, minmax(0, 1fr)); }
.s-testimonials.testimonials-featured li:first-child { grid-column: 1 / -1; }
.s-testimonials.testimonials-featured li:first-child .testimonial { padding: clamp(2rem, 6vw, 4rem); }
.s-testimonials.testimonials-featured li:first-child blockquote { font-size: clamp(1.4rem, 3vw, 2.25rem); line-height: 1.3; }
.s-testimonials.testimonials-editorial ul { grid-template-columns: repeat(2, minmax(0, 1fr)); gap: clamp(2rem, 5vw, 5rem); }
.s-testimonials.testimonials-editorial .testimonial { padding: 0; border: 0; border-radius: 0; background: transparent; }
.s-testimonials.testimonials-editorial blockquote { font-size: clamp(1.25rem, 2.4vw, 1.75rem); line-height: 1.4; }
.s-testimonials.testimonials-stacked ul { grid-template-columns: minmax(0, 46rem); justify-content: center; }
.s-testimonials.testimonials-stacked .testimonial { display: grid; grid-template-columns: minmax(0, 1fr) auto; align-items: end; gap: 1.5rem; }
.s-testimonials.testimonials-carousel ul { display: flex; overflow-x: auto; scroll-snap-type: x mandatory; padding-bottom: .75rem; }
.s-testimonials.testimonials-carousel li { flex: 0 0 min(82vw, 28rem); scroll-snap-align: start; }
.s-testimonials.testimonials-carousel .testimonial { height: 100%; }
@media (max-width: 42rem) {
  .s-testimonials.testimonials-featured ul,
  .s-testimonials.testimonials-editorial ul { grid-template-columns: 1fr; }
  .s-testimonials.testimonials-stacked .testimonial { display: block; }
}
...
.s-contact-form.contact-split { display: grid; grid-template-columns: .8fr 1.2fr; gap: clamp(2rem, 6vw, 5rem); align-items: start; }
.s-contact-form.contact-split form { grid-column: 2; grid-row: 1 / span 3; }
.s-contact-form.contact-card form { padding: clamp(1.25rem, 4vw, 2.5rem); border: 1px solid var(--border); border-radius: .75rem; background: var(--surface); box-shadow: 0 1rem 2.5rem color-mix(in srgb, var(--text) 9%, transparent); }
.s-contact-form.contact-panel { max-width: 52rem; padding: clamp(2rem, 5vw, 4rem); border-radius: .75rem; background: var(--surface); }
.s-contact-form.contact-panel form { max-width: none; }
.s-contact-form.contact-minimal input,
.s-contact-form.contact-minimal textarea { border-width: 0 0 1px; border-radius: 0; background: transparent; padding-inline: 0; }
@media (max-width: 42rem) { .s-contact-form.contact-split { display: block; } }
...
.s-footer.section-presented { max-width: none; color: var(--section-text); }
.s-footer.section-presented.section-width-narrow nav,
.s-footer.section-presented.section-width-narrow p { max-width: 48rem; }
.s-footer.section-presented.section-width-balanced nav,
.s-footer.section-presented.section-width-balanced p { max-width: 70rem; }
.s-footer.section-presented.section-width-wide nav,
.s-footer.section-presented.section-width-wide p { max-width: 90rem; }
.s-footer.section-presented a { color: var(--section-text); }
.s-footer.section-presented a:hover,
.s-footer.section-presented a:focus-visible { color: var(--section-button-hover); }
.s-footer.footer-layout-centered { text-align: center; }
.s-footer.footer-layout-centered ul { justify-content: center; }
.s-footer.footer-layout-split { display: grid; grid-template-columns: 1fr 1fr; align-items: center; gap: 1rem 3rem; }
.s-footer.footer-layout-split nav { width: 100%; }
.s-footer.footer-layout-split p { width: 100%; margin-top: 0; text-align: right; }
.s-footer.footer-layout-stacked nav + p { margin-top: 2rem; }
.s-footer.footer-layout-stacked ul { display: grid; grid-template-columns: repeat(auto-fit, minmax(10rem, 1fr)); }
.s-footer.footer-layout-minimal { padding-block: 1rem; }
.s-footer.footer-layout-minimal nav + p { margin-top: .5rem; }
@media (max-width: 42rem) { .s-footer.footer-layout-split { display: block; } .s-footer.footer-layout-split p { margin-top: 1rem; text-align: left; } }
";
