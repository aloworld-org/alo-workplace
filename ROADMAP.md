...
- [x] Mail: read/compose/reply, conversation view + flat toggle, folders/subfolders, drag-drop — *audited 2026-08-25: all present in the shipped mail surface (`MailModule` thread/flat toggle persisted per user, `FolderSidebar` nesting + create-under-parent, drag-to-folder in list and sidebar) and in daily use on mail.alomails.com; the line was stale, not the product*
- [x] Organization primitives: flags with due dates, categories/colors, archive keystroke, unread counts — *audited 2026-08-25: `FlagDueControl` + `alo:flagDue` served by the live API, category chips/picker/section shipped, archive + the flag/delete/move/mark-unread keystrokes in `MailModule`, unread badges in the sidebar*
...
- [x] Visual Sieve rule builder — *audited 2026-08-25: `FiltersSection` edits structured rules (conditions → actions) and `alo-jmap::filters` compiles them, with any out-of-office vacation, into the one managed Sieve script*
- [x] Signatures (per identity + org footer), out-of-office with scheduling — **out-of-office with scheduling done 2026-08-25**: a first and last day away on the settings screen, the reply gated on that window when a message arrives rather than by a timer that could be down on the day, and the JMAP `VacationResponse` `fromDate`/`toDate` we advertise now actually stored and reported instead of always reported as null. Org footer is live. **Per-identity signatures done 2026-08-25**: `identity_signatures` stores both RFC 8621 spellings per send address, `Identity/get` serves each identity its own signature with the account-level one as fall-back (so untouched accounts behave exactly as before), and `Identity/set` exists — update-only, signatures only, create/destroy refused because identities are provisioned. A standard client that sets a signature on one alias now actually gets it; our own web app keeps its account-level signature box unchanged
...
- [~] Localization (i18n): runtime locale mechanism built — `strings` is a proxy over the active catalog so all ~50 call sites switch language with zero changes, English fallback per key (a partial catalog never blanks), browser detection + localStorage persistence + `<html lang>`, language switcher in the account menu, full-tree remount on switch. **French and Dutch catalogs complete** (the whole surface — `fr.ts`/`nl.ts` track `en.ts` within ~2%, native quality; the old note calling NL a follow-on was stale). Follow-on: **DE catalog** (data only, mechanism ready — deliberately its own pass rather than rushed: ~6,800 lines across every module deserve the care FR got); server-synced per-user preference (currently client-side)
...
- [~] `alo-dav`: CalDAV/CardDAV over the store. **CardDAV built** (contact sync, RFC 6352) and **CalDAV built** (RFC 4791, see the two rows below): OPTIONS/PROPFIND/REPORT(multiget+query+sync-collection+free-busy-query)/GET/PUT/DELETE at `/dav/…`, HTTP Basic via `authenticate_legacy`, RFC 6578 sync-token = account modseq (a room's is a state hash), content-hash ETags, tenant-isolated (tested store + protocol), and both protocols' full client sequence recorded as wire transcripts in `docs/interop.md`. Still a module in alo-jmap; **only the crate extraction remains**. Cut: `addressbook-query` filters and a `calendar-query`'s non-time-range filters (unfiltered fallback), PROPPATCH/MKCOL/MKCALENDAR. Details in `docs/interop.md`
- [x] Events, invitations (iTIP/iMIP), free/busy, recurring events with exceptions (the interop minefield — its own test corpus): the event record and week/day/month grids; iTIP over iMIP both ways (REQUEST/REPLY/CANCEL, per-instance too) with the reading-pane invite card posting `/calendar/rsvp`; free/busy as a JSON scheduling grid and as a CalDAV `free-busy-query` (times only, by construction); `RRULE`/`RDATE`/`EXDATE` with per-occurrence overrides that sync **both ways** over CalDAV (a phone's "this event" edit, its cancellation and its removal all land), zoned expansion in the zone's wall-clock across DST, and a served `VTIMEZONE` per zone. One store expansion feeds the grid, availability and CalDAV — never two. Round-trip corpus of real Apple/DAVx⁵/Outlook fixtures (`alo-store/tests/ical_corpus.rs`)
- [x] Shared calendars, room/resource booking: a calendar per person plus shared/team calendars with owner/editor/viewer roles, each its own CalDAV collection (a viewer's is read-only on the wire); working hours + per-person time zone, so the grid separates *busy* from *outside hours* for cross-border teams; rooms and resources as tenant-owned resource calendars (admin CRUD), booked by naming the room as an attendee from the Agenda, the JSON API **or** a CalDAV PUT, with a double booking refused at write (`409`, naming the taken slot) and a room served to every member as a read-only collection carrying `CUTYPE=ROOM`
- [~] Agenda UI integrated with Mail (invite cards) and later Meet — **invite cards built** (the reading pane renders the iMIP invitation and RSVPs in place); the Meet half waits on Meet
...
- [ ] Mobile apps
- [x] ~~**MAPI-over-HTTP adapter: native Outlook**~~ — **retired, not deferred
  ([ADR 0056](docs/decisions/0056-our-own-client-on-443-is-the-product.md)).**
  alo's own client over 443 is the product. An app that works perfectly with
  somebody else's client is an app nobody opens, and the compatibility question
  turned out to be the wrong question rather than one with a wrong answer.
  ADR 0051 committed to Exchange-class access and ADR 0055 narrowed it to a
  migration bridge; both are superseded. `products/mail/alo-mapi` is deleted,
  `/mapi/*` is no longer a route, and Autodiscover offers IMAP and SMTP only.
  Checked as done because it *is* done — decided, executed and recorded — not
  because it shipped. Open standards are unaffected: IMAP, POP3, CalDAV,
  CardDAV and SMTP submission stay exactly as they are, and are how any
  third-party client connects. What was learned on the wire is kept in
  `docs/interop.md`, which is worth more than the code was
...
- [ ] A business runs on alo's own clients over 443: mail, calendar, contacts,
  chat, meet, drive and docs reached from alo's web and desktop apps and nothing
  else — no sidecar port, no per-desktop install, no "you also need to configure
  X". Restated by
  [ADR 0056](docs/decisions/0056-our-own-client-on-443-is-the-product.md). It
  previously asked for an Outlook user who never notices Exchange is gone, then
  for a business that migrates without a desktop visit; both described
  Microsoft's product rather than ours. Third-party clients keep IMAP, POP3,
  CalDAV and CardDAV — supported, simply not what this gate measures
