...
- [~] C4.1 A send is a durable job with per-recipient rows, so a crash resumes rather than restarts and **nobody is mailed twice** — idempotency on (campaign, address), enforced in the store. **Written, not yet proven.** `campaign_send.rs` + migration 0800: the job, the per-recipient ledger, and idempotency on (campaign, address) — enforced per *campaign* rather than per send, so a stop-fix-resend cannot re-mail anybody. Nine tenancy tests exist and type-check; they have never been run, because no local Postgres was available. Ticks when CI runs them green.
