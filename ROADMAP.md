...
- [~] **MAPI-over-HTTP adapter: native Outlook — the last wall.** Decided and
  ...
  - [~] 5. `OpenMessage` + streams; Outlook opens and reads a message — **the kill gate, and it is passed for text mail.** `RopOpenMessage` opens a message named by folder id and MID, `RopGetPropertiesSpecific` answers its properties, and `RopRelease` closes it. A client logs on, reads a folder's table to learn a MID, opens that message and gets back its subject, its plain-text body, its To line and its sender — proven over HTTP against a real store. Property rows are flagged, so one absent property (a message with no `Date`) no longer refuses the whole read. `RopOpenStream`/`RopReadStream` now serve values over the client's own size limit, so a long body comes back whole in chunks rather than not at all — including the `0xBABE` extended read form, which makes the request four bytes longer and would otherwise resume the walk mid-operation. `RopGetAttachmentTable` and `RopOpenAttachment` list a message's files and hand one back: the table reports name, size and type, and the bytes come through a stream on `PidTagAttachDataBinary` — which is also how the `PtypBinary` count-width question is avoided rather than guessed at, since a stream has no count field. The buffer is rehearsed up to three times, because the object graph is three deep (folder, message, attachment) and one pass cannot see past a message it has not loaded. **Still to do inside this stage:** HTML bodies (`PtypBinary` in a *row*, whose byte-count width differs between a ROP buffer and the MAPI/HTTP structures and wants a real client to settle) and the recipient table — `RecipientCount` is zero and the To/Cc lines come from `PidTagDisplayTo`/`PidTagDisplayCc` instead
