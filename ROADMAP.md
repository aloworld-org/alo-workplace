...
- [~] O1.c **Accepting a quote routes by content** — **blocked, and not on this track's code.** `billing_quote_lines` (migration 0105) carries no `product_id` by deliberate design, so that a later price change can never rewrite an offer already made. A quote therefore cannot say which catalog item a line is, and there is nothing to route on. Letting the seller choose instead does not rescue it: an order copied from those lines would name no product on any line, and `inv_so_deliver` refuses exactly those as *"a charge in words, not goods"* — so the order could never deliver anything, and would look like the feature while being an ornament. The unblocking change is one additive column on a **billing** table, filed with its shape under *Requests* in `docs/autonomy/orders/QUEUE.md`. Until it lands, a goods quote is typed into an order by hand, which is what happens today.
