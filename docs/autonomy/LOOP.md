...
| **agents-web** | `docs/autonomy/agents-web/QUEUE.md` + `agents-web/STATE.md` | the agent on the record in focus (A8.4, moved): `web/src/agents/**` and only the detail/drawer/editor views of the moved modules; `web/src/billing/**` **for AW.7's two panel mounts only** — on loan from Codex, reopened on the owner's word 2026-08-30 08:30 after Codex moved to `web/src/sites/**`; Codex is still in that checkout, so AW.7's baseline check (`git log $BASE..origin/main -- web/src/billing/`) runs before the first edit and before the push, and any commit it prints is a `LOOP HALT`. Forbidden again once the wave closes; never `web/src/shell/**`, `web/src/ds/**`; no Rust, no migrations |
...
| **verbs-a** *(parked 2026-08-30 — do not start)* | `docs/autonomy/verbs-a/QUEUE.md` + `verbs-a/STATE.md` | Billing's twenty deferred exclusions become verbs or permanent refusals: `alo-ai` + `alo-jmap` `billing_intents.rs` and new executor modules; never `web/**`; migrations **`0930`–`0949`** |
| **verbs-b** *(parked 2026-08-30 — do not start)* | `docs/autonomy/verbs-b/QUEUE.md` + `verbs-b/STATE.md` | the other twenty-seven, across Drive, Sales, Finance, Tasks, Chat, People, Projects and Sites: those modules' `*_intents.rs` and their executor twins; never `billing_intents.rs`, never `web/**`; migrations **`0950`–`0969`** |
