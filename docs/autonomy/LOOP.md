...
| **agents-web** | `docs/autonomy/agents-web/QUEUE.md` + `agents-web/STATE.md` | the agent on the record in focus (A8.4, moved): `web/src/agents/**` and only the detail/drawer/editor views of the moved modules; never `web/src/billing/**`, `web/src/shell/**`, `web/src/ds/**`; no Rust, no migrations |
| **agenda-sync** | `docs/autonomy/agenda-sync/QUEUE.md` + `agenda-sync/STATE.md` | Agenda's CalDAV cuts and launch-tier rows: `alo-store` `calendar*.rs`/`ical.rs`, `alo-jmap` `carddav.rs`/`calendar.rs`, `web/src/agenda/**` (not `DayPanel` — agents-web mounts there); `web/src/sites/**` read only; migrations **`0910`–`0929`** |
