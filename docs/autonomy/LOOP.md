...
| **verbs-a** | `docs/autonomy/verbs-a/QUEUE.md` + `verbs-a/STATE.md` | Billing's twenty deferred exclusions become verbs or permanent refusals: `alo-ai` + `alo-jmap` `billing_intents.rs` and new executor modules; never `web/**`; migrations **`0930`–`0949`** |
| **verbs-b** | `docs/autonomy/verbs-b/QUEUE.md` + `verbs-b/STATE.md` | the other twenty-seven, across Drive, Sales, Finance, Tasks, Chat, People, Projects and Sites: those modules' `*_intents.rs` and their executor twins; never `billing_intents.rs`, never `web/**`; migrations **`0950`–`0969`** |
