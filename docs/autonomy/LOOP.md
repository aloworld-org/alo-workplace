...
| **agents-web** | `docs/autonomy/agents-web/QUEUE.md` + `agents-web/STATE.md` | the agent on the record in focus (A8.4, moved): `web/src/agents/**` and only the detail/drawer/editor views of the moved modules; `web/src/billing/**` **for AW.7's panel mounts only** — released by the owner 2026-08-30, under AW.7's Codex check, and forbidden again once that wave closes; never `web/src/shell/**`, `web/src/ds/**`; no Rust, no migrations |
