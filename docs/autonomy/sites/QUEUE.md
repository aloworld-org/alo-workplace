...
- [x] S3.01c Constrained resize: each section type declares its allowed ratios and shapes, and the editor offers only those; responsive goldens at phone, tablet and desktop, and a test that no gesture can produce free positioning.
- [x] S3.01d Section palette: drag a new section in, previewed with the tenant's own content rather than lorem ipsum; keyboard path and goldens.
- [x] S3.01e Editor arc review: one browser arc from blank page to published site using only direct manipulation, checked for accessibility, mobile and the reviewable-diff property.
...
- [x] S3.02c Cost and abuse: a per-site monthly **spend** ceiling that is defaulted rather than blank, per-visitor and per-IP rate limits below it, a graceful unavailable message that offers the contact form, and the tenant told when it is hit.
- [x] S3.02d Source-adding UI: the screen that publishes a source to the bot says *anyone on the internet will be able to read this* above the button, every time; the ceiling is set in the same screen the bot is switched on.
- [x] S3.02e Visitor chat UI: on-site widget, keyboard-accessible, mobile, citations as links, and an honest empty/unavailable state.
- [x] S3.02f Appearance and voice model (ADR 0040 §5): the widget inherits the site's preset palette, logo and favicon; the tenant owns the welcome message, bot name and avatar, up to three suggested questions, a tone note, launcher position and icon, and the offline message. Colour is a choice among the site's own palette roles — no free-form colours, no custom CSS, no custom fonts. Validation tests proving no stored value can produce failing contrast, and a test that nothing in the tone note widens what §1 and §2 allow.
- [x] S3.02g Appearance UI: a live preview of the real widget beside the fields, a written default welcome message rather than an empty box, suggested questions drafted from the published site's own headings and editable, and an accessibility check shown in the screen rather than discovered later.
...
- [x] S3.03b Booking from the conversation: create the meeting, send the confirmation, put it in the visitor's calendar, and include a cancellation link — with the reversible-only rule enforced in code rather than in the prompt.
- [x] S3.03c **CRM-owned**: a public seam to create a contact and a lead from a site conversation.
- [x] S3.03d Lead capture through that seam, storing aggregate attribution only and no individual visitor journey.
- [x] S3.03e What the bot did: a tenant-facing transcript showing each action, the fact it used and the page that fact came from.
...
- [x] S3.04b Hold-with-expiry: capacity is reserved *before* payment and released if the buyer does not finish; concurrency tests proving two simultaneous buyers cannot oversell the last seat. This is the first commit of the wave, not hardening.
- [x] S3.04c Hosted payment handoff: a provider adapter with a fixture provider and no external calls in tests, an order → payment-reference → paid state machine, idempotent webhook handling, and a test that no card data can reach alo. Mollie or Adyen ahead of Stripe.
- [x] S3.04d Fulfilment: the ticket by email and in the buyer's calendar, the contact in CRM and the invoice in Billing, each through its owned seam. (Shipped: ticket page + .ics, invoice, CRM — the email is S3.04h; alo has no outbound-mail-to-strangers path to send it with.)
- [!] S3.04e Place-of-supply VAT for event tickets as a rules table with tests — reviewed by a tax professional before this item is taken, because wrong here is wrong in Finance years later. blocked: the review is a human-arranged prerequisite and none is on record (2026-08-16); the loop cannot supply it — see STATE.md for what the review must cover.
- [x] S3.04f1 Public checkout doors on the store: `site_public_shop` — the offer priced by the Billing seam at every read (never a stored copy), begin-checkout in the only safe order (typo gate → hold → order), payment open/settle wrappers and the webhook target, the return-page status door with the ticket token; Host-anchored walls on every verb; DB tests. (Split from S3.04f 2026-08-16: one turn cannot hold the store doors, the public pages, the section renderer and the editor mirror at full depth — the S2.12c/S2.15c precedent.)
- [x] S3.04f2 Shop pages and checkout on alo-sites: the `tickets` typed section (Rust model + renderer + goldens + mobile CSS + EN/FR/NL strings) rendering the offer link; live `/tix` listing and `/tix/{event}` offer pages (no-store, priced through the S3.04f1 doors); POST checkout → hosted-payment redirect via a `SitePaymentProvider` in AppState (unconfigured deployment answers honestly); the return page with fetch-on-return settle and the ticket link; `POST /_alo/pay` webhook (doorbell only — status always fetched); rate limits + honeypot like the booking door; in-process wire tests incl. Host isolation.
- [x] S3.04f3 Shop section in the web editor: the section-type mirror, picker entry with thumbnail, event management UI (create/capacity/delete over the existing `/sites` ticket routes — wire them if S3.04b left them store-only), i18n en/fr/nl; tsc/eslint/build clean; manual click-path journaled.
- [x] S3.04g The arc: a visitor asks the bot about an event, is offered tickets at a price read from the catalog, pays on the provider's page, and the ticket, the contact and the invoice all exist. No price the model invented.
- [x] S3.04h The ticket by email (deferred from S3.04d): PREREQUISITE, in this item, not elsewhere — an ADR first, because alo has never emailed a stranger automatically (ADR 0034 routes even purchase orders through a human's Drafts) and an automated outbound path needs its abuse story (sender identity, DKIM domain, rate caps, bounce handling) decided before code. Then the narrowest seam that decision allows, the ticket mail through it, and tests that prove a foreign tenant's sale can never mail through another tenant's identity. NO live sending in the loop, ever — structural verification only.
...
- [x] S3.05b Propose the configuration: from a sentence about the business, draft the catalog, the VAT treatment per item and the shipping, and present it for approval with every guess flagged — the screen where Odoo loses the customers who cannot afford a consultant. Fixtures only. (Split 2026-08-16, the S3.04f1 precedent: this item shipped the proposal engine in alo-ai — the closed envelope, the parser-enforced flags and the fixture tests; the route is S3.05b2 and the screen S3.05b3.)
- [x] S3.05b2 Proposal route: POST `/sites/shop-config/propose` (description → the S3.05b envelope, returned for review and never applied server-side; typed "unconfigured" the UI can branch on, the S1.28a shape); owner/workspace auth only — NOT the per-site editor grant (S2.03a: site editors must never see Billing-side facts, and this proposal names prices and VAT); Problem errors; wire transcript incl. 401/422/unconfigured.
- [x] S3.05b3 Proposal screen: the describe-your-business box → the proposal as an approval list with every guess visibly flagged (VAT always shown as a guess to confirm, blank prices as required fields, shipping's flagged blank likewise), edit-then-approve, approval applying ONLY through the already-owned Billing product / ticket-event / shop-settings routes (no new write path); unconfigured deployments keep the manual path beside it; i18n en/fr/nl; manual click-path journaled.
- [x] S3.05c Shop UI for stock items, sharing the checkout built in wave one.

- [x] S3.06a Wave review, security reconciliation (split from S3.06 2026-08-16, the S2.16 precedent): the authenticated surface Wave 3 added, re-walked against the site-editor grant at BOTH mounts. The chatbot doors held (every `chat-*` route is owner-only); the commerce doors did not: the ticket/shop pickers answered a restricted collaborator with the tenant's whole active price list, and the write verbs (event create/capacity/delete, shelf add/remove, delivery rate) let an outside contractor change what the business sells. Narrowed to the owner with verbatim refusals, the editor-readable lists kept (public-by-construction facts the page editor needs), the matrix pin test extended over the new surface, the web screens degrading honestly for a collaborator, and the as-built matrix updated.
- [x] S3.06b Wave review, accessibility, responsiveness and language parity: every Wave-3 screen (tickets, shop, shop setup, assistant appearance, palette/inline editing) against `docs/design/ux-principles.md` and the keyboard/screen-reader basics — dialog contract, landmarks, accessible names, phone widths — fixing what it finds; EN/FR/NL parity re-checked (the catalog ratchet is green, the wave's new strings are the risk); includes the S3.06a follow-up that read-only commerce screens surface their state to assistive tech.
- [x] S3.06c Wave review, as-built docs and reconciliation: `docs/design/sites.md` reconciled with what Wave 3 actually shipped, features.md [S3] table, CHANGELOG sweep, human-inbox summary (payment-provider keys, the ticket-mail sender identity, Caddy prefixes, the AI key, and the open decisions this wave flagged — the blocked S3.04e tax review first among them).
- [x] S3.06d1 Wave review, final arcs: the three strands (direct-manipulation editing; the bot that answers and acts; tickets + stock commerce) driven end to end on the real local stack — two real processes, real curl, real database rows, the services' own sweeps — with transcripts and byte budgets; fix what it finds. (Split from S3.06d 2026-08-16, the S2.16b→b2 precedent: one turn cannot hold three real-stack arc strands AND the full browser walk with its fixes at full depth.)
- [x] S3.06d2 Wave review, the walk a test cannot do: every sites screen opened in a real browser at 360px and driven by keyboard alone (the S2.16b2 playwright-core rig), reduced-motion and contrast included, the Wave-3 screens first (tickets, shop, shop setup, assistant, palette/inline editing); fix what it finds; then the wave closes.
