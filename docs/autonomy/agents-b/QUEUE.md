...
- [x] AB.4 ★ **Tasks**: reads — `my_plate`, `overdue_by_owner`, `thread_actions` kept, plus a board's open tasks, a task lookup; writes — the existing four kept, complete a task, reassign, with previews.
- [x] AB.5 ★ **Agenda**: reads — `whats_on`, `am_i_free`, `find_a_time`, `meeting_prep` kept, plus an event lookup, a colleague's availability where shared; writes — `create_event`, `reschedule_event` kept, cancel, respond to an invitation, with previews.
- [x] AB.6 Wave review, as AA.6, for these five. Then `LOOP COMPLETE`.
