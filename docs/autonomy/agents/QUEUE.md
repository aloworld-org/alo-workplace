...
- [x] A8.3 Goals: a goal record with Ask alo's plan, steps, progress, one
  ...
  and Agenda.
- [x] A8.4 *(done 2026-08-29 in track **agents-web** — `docs/autonomy/agents-web/QUEUE.md` + `STATE.md`, wave AW: the `RecordAgentPanel` and its mount in all sixteen moved modules, walked at desktop and phone width in AW.6. Billing's own mount stayed with the owner, whose directory it is. The block below is history)* `[web]` The agent on the record in focus in every moved module. *(blocked 2026-08-29: the record-in-focus surfaces are the module detail views in `web/src/billing|crm|inventory|tasks|projects|…` — all outside this track's web writ of `web/src/chat/**` + new `web/src/agents/**`, and the ds loop is live in those very dirs today (its last three commits edited tasks/projects/inventory web files). A component parked in `web/src/agents/**` with no module rendering it would be a stub. Unblocks when the module web dirs are free and the writ is widened — the backend half (origin on records, intents returning it) shipped in A4.5.)*
