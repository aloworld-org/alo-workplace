...
- [x] D2.09b Migrate **inventory** — the half of D2.09 that was cut, said as its own item. Ten `.tsx` (the catalog, the stock list, the two order lists, the two order editors, the line grid, the consignments, the move-history dialog and the scanner) over a 626-line stylesheet. Prerequisites, copied here rather than left in a guide: **(a)** `ds/` still has no multi-line control, so `.textarea` stays local — inventory is the **seventh** area waiting, after billing ×3, crm, finance and hr ×2; **(b)** `parts.tsx` exports a module-local `Field` and a `StatusChip` whose five tones (`neutral`/`info`/`good`/`warn`/`muted`) are the return type of `format.ts`'s `poStatusTone` and `soStatusTone`, which map eleven order states onto them — `ds/Badge` has three, so the tone vocabulary stays the module's exactly as hr's `StateBadge` kept its, and only the drawing is adopted; **(c)** the module hand-rolls **two** dialogs (`.modalWide` for the movement history, `.scanModal` for the scanner) with no focus trap between them, which is the behaviour `ds/Modal` exists for and the reason this is a migration rather than a restyle; **(d)** `.search` puts no icon inside the field, unlike hr's, so it is a plain `ds/Input` and not an exemption.
- [x] D2.10 Migrate **platform** — same loop: adopt, delete, remove the line, screenshot. **Projects was taken off `redefined.ts` by somebody outside the loops** (`c41851be refactor(projects): unify actions and migrate styles to Tailwind`, landed 2026-08-19 while D2.09 was being gated), which is the case LOOP describes: a commit in an area no journal claims. Read `web/src/projects/**` before assuming it is done to this queue's standard — an area that stops declaring a primitive has satisfied the ratchet, which is not the same as having adopted the components — and file what is missing as its own item rather than reopening theirs.
- [x] D2.10b Adopt the components in **projects** — the audit D2.10 ordered found the area off the ratchet but not on the design system: it is billing's problem again, Tailwind recipes `primitives.test.ts` cannot see. What the read found, so the item does not have to re-find it: **(a)** `parts.tsx`'s `DialogFrame` is a hand-rolled modal carried by all six dialogs (Client, EditProject, Entry, Milestone, NewProject, Template) — overlay, Escape and outside-click, but **no focus trap**, which is the behaviour `ds/Modal` exists for; **(b)** `parts.tsx` exports a local `Field` (label not bound to the control, hint/error never announced — inventory's defect exactly) and a `WeekChip` recipe whose four tones stay the module's vocabulary, only the drawing adopted; **(c)** ~20 bare `<input>`/`<select>`/`<textarea>` across the dialogs and ReportView/PlanView/TemplateDialog should be `ds/Input`/`ds/Select` (`textarea` stays local — still no multi-line control in `ds/`, projects is the eighth area waiting); **(d)** five hand-rolled `<table>`s (ApprovalsView, ProjectsView, ReportView, WeekView ×2) against `ds/Table`; **(e)** fifteen `bg-[var(--…)]`-style escapes, several reaching the private raw scale (`--navy-50`, `--navy-600`) — respell onto semantic utilities, or add the token if one is genuinely missing.
- [x] D2.11 Migrate **tasks** — one commit, same loop: adopt, delete, remove the lines, screenshot. **Tasks too has left the ratchet by outside hands** (no `.module.css` remains in the area): D2.10's projects audit found ratchet-satisfaction is not component-adoption, so read `web/src/tasks/**` the same way — `parts.tsx` first — and turn this item into the adoption gaps actually found. The audit's findings and what was adopted are in STATE; the detail panel was cut to D2.11b.
- [x] D2.11b Migrate the **task detail panel** — the half of D2.11 that was cut, said as its own item. `TaskDetail.tsx` (~800 lines) is a slide-over, not a centred dialog, and it has **no Escape handling and no focus trap at all** — the exact defect `ds/Modal` fixes, but `Modal` cannot express it yet: its panel centres where the detail anchors right at full height, and its title is an `<h2>` where the detail's is an editable input. Either `Modal` learns a `side` variant with a ReactNode header, or the trap and stack move into a hook both share — decide by reading `Modal` first; its modal *stack* (added in D2.11) must keep working over a detail panel that opens dialogs. Also in the file: ~10 bare `<input>`/`<select>`/`<textarea>` inline edits for `ds/Input`/`ds/Select`/`ds/Field` (the `textarea`s stay bare — still no multi-line control; tasks is the ninth area waiting). And `TaskToolbar.tsx` hand-rolls a Dropdown popover (outside-click + Escape, `menuitemradio`) beside `ds/Menu` and `ds/ChoicePicker` — read all three before deciding which absorbs which; its semantics are already right, so it is an adoption question, not a defect.
- [x] D3.01 Wave review: everything outside `web/src/sites/**` is off `ds/redefined.ts`; walk the product in a browser at desktop and phone width; check the CSS actually shipped has fallen from its 252 KB; and record in `CHANGELOG.md` what a user would notice.
