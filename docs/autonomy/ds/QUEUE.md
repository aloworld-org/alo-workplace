...
- [ ] D2.11 Migrate **tasks** — one commit, same loop: adopt, delete, remove the lines, screenshot. **Tasks too has left the ratchet by outside hands** (no `.module.css` remains in the area): D2.10's projects audit found ratchet-satisfaction is not component-adoption, so read `web/src/tasks/**` the same way — `parts.tsx` first — and turn this item into the adoption gaps actually found.
