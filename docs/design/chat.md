...
| `GET /chat/people?q=` | Colleagues whose address matches — **a search, never a listing**. Two characters minimum, capped at 25, the caller excluded, tenant-bounded, wildcards escaped so `%` cannot mean "everyone". It exists because starting a conversation needs a way to find a person and every other route here is admin-gated or a personal address book; it is the least surface that makes that possible |
