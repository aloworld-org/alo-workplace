...
| Employee records: personal data, role, team, manager | **Yes** — B6.02a, three doors, field-level scoping. **One gap, found on the wire at B6.12b** (`docs/autonomy/STATE.md`): employment **terms can only be stated when the person is created**. `append_hr_employment` — the store function this note's two-table argument is built around — has exactly one caller, `POST /hr/employees`; `PATCH` refuses terms as a rule ("terms are appended, not edited"), and no route appends. So a promotion, a move to four days a week or a corrected working pattern needs archive → unlink the user → recreate, and a person created without terms can book no leave at all. `POST /hr/employees/{id}/employments` is the missing door |
